...
class StressEngine(
     ...
     * @param samplingHz Sampling rate of the PPG data in Hz (e.g., 50).
     */
    fun processChunk(
        ...
        val hrMean: Double? = when {
            hrVals.isNotEmpty() -> hrVals.average()
            else -> lastValidHr
        }.also { mean ->
            if (mean != null && mean in MIN_HEART_RATE_BPM..MAX_HEART_RATE_BPM) {
                lastValidHr = mean
            }
        }
        ...
        // Baseline window (first 10 min)
        if (baselineWindow.size < baselineRows) {
            baselineWindow += point
        }

        // Sliding window (always last baselineRows points)
        slidingWindow += point
